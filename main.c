...
#include <math.h>       // Para usar calculos matemáticos
...
int main(int argc, char* argv[])
{
    ...
    memcpy(pic[DST].img , pic[PAL].img, pic[1].width*pic[1].height*3);
    printf("Processando...");
    ...
    while(1){
        ...
        RGB* s1 = &pic[SRC].img[pos1];
        RGB* s2 = &pic[SRC].img[pos2];
        RGB* d1 = &pic[DST].img[pos1];
        RGB* d2 = &pic[DST].img[pos2];

        if(dist(s1,d1) + dist(s2,d2) > dist(s1,d2) + dist(s2,d1)){
            RGB aux = *d1;
            pic[DST].img[pos1] = *d2;
            pic[DST].img[pos2] = aux;
            tent = 0;
            tot++;
        }else tent++;
            if(tent > 300){
